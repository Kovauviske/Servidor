...
ctrlShow[2001,false];
if((lbCurSel 2022) == -1) exitWith {hint "Ninguém foi selecionado!";ctrlShow[2001,true];};
...
if(!life_use_atm) exitWith {hint "Recentemente, você roubou o banco! Você não pode dar dinheiro.";ctrlShow[2001,true];};
if(!([_amount] call TON_fnc_isnumber)) exitWith {hint "Você não digitou um formato de número real.";ctrlShow[2001,true];};
if(parseNumber(_amount) <= 0) exitWith {hint "Você precisa digitar uma quantidade real que você quer dar.";ctrlShow[2001,true];};
if(parseNumber(_amount) > life_cash) exitWith {hint "Você não tem muito para dar!";ctrlShow[2001,true];};
if(isNull _unit) exitWith {ctrlShow[2001,true];};
